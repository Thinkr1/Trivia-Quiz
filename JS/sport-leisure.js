function formatQuestionText(question) {
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    const response = await fetch("../JSON/sport-leisure.json");
    ...
    questionElement.setAttribute("SLanswer", randomQuestionObj.answers);
  });
  ...
  function skipQuestion() {
    showNQ();
    const questionElement = document.getElementById("question");
    const questionAnswer = questionElement.getAttribute("SLanswer");
  ...
  function updateAnswerCounters(message) {
    ...
    let SLuserAnswers = JSON.parse(localStorage.getItem("SLuserAnswers")) || {
      correct: 0,
      incorrect: 0,
      skipped: 0,
    };
    if (message === "Correct!") {
      SLuserAnswers.correct++;
    } else if (message === "Skipped") {
      SLuserAnswers.skipped++;
    } else {
      SLuserAnswers.incorrect++;
    }
    const totalAnswerCount =
      SLuserAnswers.correct + SLuserAnswers.incorrect + SLuserAnswers.skipped;
    answerCountElement.textContent = `${SLuserAnswers.correct} correct, ${SLuserAnswers.incorrect} wrong and ${SLuserAnswers.skipped} skipped (${totalAnswerCount} total)`;
    localStorage.setItem("SLuserAnswers", JSON.stringify(SLuserAnswers));
  }
