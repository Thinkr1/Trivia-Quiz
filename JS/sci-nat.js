function formatQuestionText(question) {
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    const response = await fetch("../JSON/sci-nat.json");
    ...
    questionElement.setAttribute("SNanswer", randomQuestionObj.answers);
  });
  ...
  function skipQuestion() {
    showNQ();
    const questionElement = document.getElementById("question");
    const questionAnswer = questionElement.getAttribute("SNanswer");
  ...
  function updateAnswerCounters(message) {
    ...
    let SNuserAnswers = JSON.parse(localStorage.getItem("SNuserAnswers")) || {
      correct: 0,
      incorrect: 0,
      skipped: 0,
    };
    if (message === "Correct!") {
      SNuserAnswers.correct++;
    } else if (message === "Skipped") {
      SNuserAnswers.skipped++;
    } else {
      SNuserAnswers.incorrect++;
    }
    const totalAnswerCount =
      SNuserAnswers.correct + SNuserAnswers.incorrect + SNuserAnswers.skipped;
    answerCountElement.textContent = `${SNuserAnswers.correct} correct, ${SNuserAnswers.incorrect} wrong and ${SNuserAnswers.skipped} skipped (${totalAnswerCount} total)`;
    localStorage.setItem("SNuserAnswers", JSON.stringify(SNuserAnswers));
  }
